(function() {
  ...
  const DEBUG = urlParams.get('msk_debug') === '1';
  
  // Détecte si c'est un produit test via le slug (préfixe test-)
  function isTestProduct() {
    const pathParts = window.location.pathname.split('/').filter(p => p);
    const pIndex = pathParts.indexOf('p');
    if (pIndex !== -1 && pathParts[pIndex + 1]) {
      return pathParts[pIndex + 1].startsWith('test-');
    }
    return false;
  }
  
  const IS_TEST = isTestProduct();
  ...
  // Banner de mode
  if (DEBUG) {
    const modeColor = IS_TEST ? '#f97316' : '#16a34a';
    const modeText = IS_TEST ? '🟠 MySellKit TEST Mode' : '🟢 MySellKit LIVE Mode';
    console.log(`%c${modeText}`, `background: ${modeColor}; color: white; padding: 8px 16px; border-radius: 4px; font-size: 14px; font-weight: bold;`);
    log.info('Environment:', IS_TEST ? 'TEST' : 'LIVE');
  }
  
  // ========== CONFIGURATION DYNAMIQUE ==========
  const CONFIG = {
    API_BASE: IS_TEST 
      ? 'https://app.mysellkit.com/version-test/api/1.1/wf'
      : 'https://app.mysellkit.com/api/1.1/wf',
  ...
  if (DEBUG) {
    ...
    log.info('Stripe Key:', CONFIG.STRIPE_KEY.substring(0, 20) + '...');
  }
  ...
  function getProductIdFromURL() {
    ...
    if (pIndex !== -1 && pathParts[pIndex + 1]) {
      ...
      if (IS_TEST) {
        // En mode test, on garde le slug complet (test-xxxxxx)
        log.info('Product ID (test mode):', slug);
        return slug;
      } else {
        // En mode live, on extrait juste l'ID (sans le préfixe test-)
        const id = slug.replace(/^test-/, '');
        if (/^[a-z0-9]{5,8}$/.test(id)) {
          log.info('Product ID (live mode):', id);
          return id;
        }
      }
    }
